...
export default function LoginPage() {
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          ...
          description: 'تم إرسال رابط استعادة كلمة المرور إلى بريدك الإلكتروني بنجاح',
        });
      ...
      toast({
        title: 'خطأ غير متوقع',
        description: 'حدث خطأ أثناء محاولة إرسال رابط الاستعادة',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-100 via-blue-50/40 to-slate-200 dark:from-[#070a12] dark:via-blue-950/20 dark:to-[#090d16] relative transition-colors" dir="rtl">
      ...
      <Card className="w-full max-w-md shadow-2xl border-border/80 dark:border-slate-800/80 bg-card/95 dark:bg-[#0c1322]/95 backdrop-blur-md rounded-2xl sm:rounded-3xl overflow-hidden">
        <CardHeader className="space-y-2 text-center pb-3 pt-6">
          ...
          <CardTitle className="text-2xl font-bold font-amiri text-foreground tracking-wide">النقل الدولي</CardTitle>
          <CardDescription className="text-sm text-muted-foreground">تسجيل الدخول إلى نظام إدارة الشحنات</CardDescription>
        </CardHeader>
        ...
        <CardContent className="px-6 pb-6 pt-2">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5 text-right">
              <label className="text-sm font-semibold text-foreground/90">البريد الإلكتروني</label>
            ...
            {/* Password Field */}
            <div className="space-y-1.5 text-right">
              <label className="text-sm font-semibold text-foreground/90">كلمة المرور</label>
              <div className="relative flex items-center">
                <Input
            ...
            <div className="text-center text-xs sm:text-sm text-muted-foreground pt-3 border-t border-border/40">
              ليس لديك حساب؟{' '}
              <Link href="/signup" className="text-sky-500 hover:text-sky-400 dark:text-sky-400 dark:hover:text-sky-300 font-semibold hover:underline mr-1">
                إنشاء حساب جديد
              </Link>
            </div>
          </form>
      ...
      <Dialog open={showForgotDialog} onOpenChange={setShowForgotDialog}>
        <DialogContent className="sm:max-w-md" dir="rtl">
          <DialogHeader className="text-right">
            <DialogTitle className="text-xl font-bold font-amiri">استعادة كلمة المرور</DialogTitle>
