    ...
    if (!doc) {
      return (
        <button
          ...
          title="انقر لرفع وتسجيل هذه الوثيقة"
        >
      ...
      return (
        <div className="flex flex-col gap-1 p-1.5 rounded-xl bg-destructive/10 border border-destructive/30 text-destructive">
          ...
          <div className="flex items-center justify-between text-[10px] font-semibold">
            <span>منتهية ({Math.abs(days)} يوم)</span>
    ...
    return (
      <div className="flex flex-col gap-1 p-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/25 text-emerald-700 dark:text-emerald-300 group hover:border-emerald-500/50 transition-all">
        ...
        <div className="flex items-center justify-between text-[10px] font-medium">
          <span className="text-emerald-600 dark:text-emerald-400">سارية ({days} يوم)</span>
    ...
    <div className="space-y-4" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 items-center justify-between bg-card border border-border/70 p-3 rounded-2xl shadow-xs">
        <div className="flex items-center gap-2 w-full sm:w-auto flex-1 max-w-md">
          <div className="relative w-full">
            ...
            <Input
              type="text"
              placeholder="بحث بالترقيم، اللوحة، أو الموديل..."
              value={searchQuery}
      ...
      <Card className="rounded-2xl border border-border/80 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-right min-w-[950px]">
            <thead>
              <tr className="bg-muted/40 border-b border-border/70 text-xs font-semibold text-muted-foreground">
                <th className="p-3.5 text-right w-44 sticky right-0 bg-muted/60 backdrop-blur-sm z-20 border-l border-border/60">
                  المركبة والترقيم
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>التأمين</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Assurance</span>
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>الفحص التقني</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Visite Technique</span>
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>البطاقة الرمادية</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Carte Grise</span>
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>رخصة النقل</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Autorisation / CMR</span>
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>شهادة التبريد</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Certificat ATP</span>
                </th>
                <th className="p-3.5 text-center min-w-[155px]">
                  <span>التاكوغراف</span>
                  <span className="block text-[10px] text-muted-foreground/80 font-normal">Tachygraphe</span>
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-16">
                    ...
                    <p className="text-xs text-muted-foreground">جاري تحميل مصفوفة الأسطول والوثائق...</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={7} className="text-center py-16 text-muted-foreground">
                    ...
                    <p className="text-sm font-semibold text-foreground">لا توجد نتائج مطابقة للفلتر المحدد</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      جرّب تغيير كلمات البحث أو إعادة ضبط الفلترة.
                    </p>
                  </td>
                </tr>
              ) : (
                filteredRows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      {/* Vehicle Column (Sticky Right) */}
                      <td className="p-3.5 sticky right-0 bg-card z-10 border-l border-border/60 shadow-xs">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[11px] text-muted-foreground truncate flex items-center gap-1 mt-0.5">
                              <span>{isTruck ? 'رأس شاحنة' : 'مقطورة'}</span>
                              <span>•</span>
                              <span>{row.model}</span>
                            </div>
                          </div>
